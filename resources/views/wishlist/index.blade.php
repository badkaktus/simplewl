<x-app-layout>
    <div class="py-6 font-bold">
        ...
        <div id="tooltip-wishlist-vision" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
            @if($wishlist->is_private)
                Your wishlist is private. Only you can see it.
            @else
                Your wishlist is public. Everyone can see it.
            @endif
            Click to change.
        ...
        <a
            class="ml-1 cursor-pointer align-text-bottom"
            data-tooltip-target="tooltip-wishlist-vision"
            href="{{ route('wishlist.changeVisibility', ['name' => $user->name, 'slug' => $wishlist->slug]) }}"
        >
            @if($wishlist->is_private)
                <svg xmlns="http://www.w3.org/2000/svg" height="20" width="25" viewBox="0 0 640 512" class="inline">
            ...
            @else
                <svg xmlns="http://www.w3.org/2000/svg" height="20" width="22.5" viewBox="0 0 576 512" class="inline ml-0.5">
                <!--!Font Awesome Free 6.5.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
                <path
